...
| "not a PRAHARI recording: …" or "the .gz file is damaged or incomplete" | The chosen file is not a recording, or its compressed data is broken from the start. | Open a `*.prs.jsonl.gz` produced by `prahari run`; copy the file again if it was damaged in transfer. |
