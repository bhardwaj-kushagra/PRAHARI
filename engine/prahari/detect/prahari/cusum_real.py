...
`statistic: z` (legacy "minus conformal" ablation, P7-12) accumulates the signed z from a Gaussian-form QCC with
k = `k_z` instead of −ln p with k = `k_node`, as the report simulation's `G_fast` source.
"""
...
class CusumReal(Stage):
    ...
    def reset(self, ctx) -> None:
        ...
        self._buf = TuningBuffer(int(p["tune_start_min"]), int(p["tune_end_min"]), int(p["cm_pad_min"]),
                                 ctx.tick_minutes)
        self._z = p.get("statistic", "neglogp") == "z"
        self._k = float(p["k_z"] if self._z else p["k_node"])

    def step(self, sc: Scores, ctx) -> Candidates:
        p, t = self.params, ctx.t
        if self._z:
            if sc.z is None:
                raise ValueError("cusum statistic 'z' needs Scores.z (qcc form robust_z)")
            s = np.asarray(sc.z, dtype=float)                    # legacy ablation — signed z increment
        else:
        ...
        if self._buf is not None and self._buf.ready(t):
            self._h, self._at_cap = tune_from_buffer(self._buf, p, self._k, self._ref_ticks)
            self._tuned = True
            self._buf = None
            self._G[:] = 0.0
            self._ref[:] = 0
        ...
        G_pre = np.maximum(0.0, self._G + s - self._k)          # M28 — G ← max(0, G − ln p − k)
    ...
    def snapshot(self) -> dict:
        return {"h": round(self._h, 3), "tuned": self._tuned, "at_cap": self._at_cap,
                "h_default": self.params["h_default"], "k": self.params["k_z" if self._z else "k_node"],
                **({"statistic": "z"} if self._z else {})}
