...
| Edge clusters candidates by arrival minute | Phase 8 | In `gateway_outage` node 41's candidate, held 23 minutes by the outage, lands in the same 30-minute window as node 31's; the fire is confirmed at 14:48 instead of 15:14 with a perfect link (SIM) | The Phase 6 cluster stage uses the current minute as each candidate's time; with the perfect-link stub arrival and detection coincide, so this only shows once the radio delays frames. Detail: `DECISIONS.md` P8-11 | Carry each candidate's detection minute in `Delivered` (additive) and let the cluster window use it (a small change to the accepted cluster stage — needs approval) | On approval |
